...
parser.add_argument("--hidden-layer-num", default=16, type=int)
...
if __name__ == "__main__":
    ...
    memory_time_info = {'hidden-layer-size': args.hidden_layer_size, 'hidden-layer-num': args.hidden_layer_num,
                        'tengrad': {'batch-size': [],'time':[], 'avg-space':[], 'min-space':[], 'max-space':[]},
                        'block-ngd':{'batch-size': [],'time':[], 'avg-space':[], 'min-space':[], 'max-space':[]},
                        'exact-ngd':{'batch-size': [],'time':[], 'avg-space':[], 'min-space':[], 'max-space':[]},
                        'sgd':{'batch-size': [],'time':[], 'avg-space':[], 'min-space':[], 'max-space':[]}}

    for batch_size in args.batch_sizes:
        train_data, test_data = train_data[:,:batch_size], test_data[:, :batch_size]
        ...
        if args.use_tengrad:
            '''Run tengrad'''
            start_time = time()
            f = lambda : train(layers=[feature_num, *([args.hidden_layer_size]*args.hidden_layer_num), 1],
                                                    activations=['sigmoid' for i in range(args.hidden_layer_num+1)],
            ...
            tengrad_memory_info = memory_usage(f, interval=1e-3, timeout=10000 )
            end_time = time()
            time_taken = end_time-start_time

            memory_time_info['tengrad']['batch-size'].append(batch_size)
        ...
        if args.use_sgd:
            ...
            sgd_memory_info = memory_usage(f, interval=1e-3, timeout=10000)
            end_time = time()
            time_taken = end_time-start_time

            memory_time_info['sgd']['batch-size'].append(batch_size)
        ...
        if args.use_block_ngd:
            ...
            blockwise_ngd_memory_info = memory_usage(f, interval=1e-3, timeout=10000)
            end_time = time()
            time_taken = end_time-start_time

            memory_time_info['block-ngd']['batch-size'].append(batch_size)
        ...
        if args.use_exact_ngd:
            ...
            exact_ngd_memory_info = memory_usage(f, interval=1e-3, timeout=10000)
            end_time = time()
            time_taken = end_time-start_time

            memory_time_info['exact-ngd']['batch-size'].append(batch_size)
    ...
    for method in ['tengrad', 'sgd', 'exact-ngd', 'block-ngd']:
        ...
        if (method == 'tengrad' and args.use_tengrad) or (method == 'sgd' and args.use_sgd) or (method == 'block-ngd' and args.use_block_ngd) or (method == 'exact-ngd' and args.use_exact_ngd):
            if args.analyze_time:
                ...
                plt.plot(memory_time_info[method]['batch-size'], (memory_time_info[method]['time']), label=method)
            ...
            if args.analyze_memory:
                plt.subplot(fig_num,1,current_fig)
                plt.plot(memory_time_info[method]['batch-size'], (memory_time_info[method]['max-space']), label=method)
                plt.xlabel('batch size')
